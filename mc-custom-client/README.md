...
- **設定メニュー（Mキー）**: ゲーム中に **M** を押すとスライダー付きの設定画面が開きます。ワールドは一時停止しないので、動かしながらその場で効果を確認できます。
  - **移動速度**: x0.25 〜 x3.00（初期値 x1.00）。歩行・ダッシュなど「入力の強さ」を変えます。
  - **Velocity(上下)**: x0.50 〜 x2.50（初期値 x1.00）。ジャンプ・落下の速さ（上下方向の速度）だけに毎tick掛かる倍率です。1.0より大きいとジャンプが高く・落下が速く、小さいとふわっとした動きになります。
    - 左右前後（水平方向）にはあえて効かせていません。水平速度全体に倍率を掛けると、方向転換した直後でも「元の進行方向の勢い」まで増幅されてしまい、旋回中にどんどん直進方向へ流されて曲がれなくなる問題があったため、この形にしています。加速しながらの方向転換は移動速度スライダー＋バニラの操作性のみに委ねられるので、通常のダッシュと同じ感覚で曲がれます。
    - 暴走防止のため、上下速度の大きさは内部で頭打ち（クランプ）しています。
  - 「リセット」ボタンで両方とも初期値に戻せます。「閉じる」または Esc で閉じます。
...
└── src/main/
    ...
    │   ├── VelocityController.java # Velocity倍率の状態管理
    │   ├── gui/
    │   │   └── SpeedMenuScreen.java   # Mキーで開く設定メニュー(スライダー)
    │   ├── hud/
    │   │   └── MinimapRenderer.java   # 右上のミニマップ描画
    │   └── mixin/
    │       └── PlayerSpeedMixin.java  # 移動速度・Velocityを実際に変更するMixin
    └── resources/
        ├── fabric.mod.json
