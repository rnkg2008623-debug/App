...
- **設定メニュー（Mキー）**: ゲーム中に **M** を押すとスライダー付きの設定画面が開きます。ワールドは一時停止しないので、動かしながらその場で効果を確認できます。
  ...
  - **Velocity(加速度)**: x0.10 〜 x3.00（初期値 x1.00）。水平方向（前後左右）に限り、「その速さに到達するまでの速度の変化しやすさ」を変えます。実装上は、バニラ物理がそのtickで計算した速度の変化量(delta)そのものに倍率を掛けています。単純に速度全体へ倍率を掛ける方式だと、方向転換した直後でも元の進行方向の勢いまで増幅されてしまい「曲がれなくなる」問題があったため、この方式にしています。大きくするほど加速も方向転換も素早くキビキビ動き、小さくすると氷の上のように滑って曲がりにくくなります。ジャンプ・落下（上下方向）には一切影響しません。
...
└── src/main/
    ...
    │   ├── VelocityController.java   # 加速度倍率の状態管理
