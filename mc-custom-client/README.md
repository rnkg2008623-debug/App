...
- **設定メニュー（Mキー）**: ゲーム中に **M** を押すとスライダー付きの設定画面が開きます。ワールドは一時停止しないので、動かしながらその場で効果を確認できます。
  ...
  - **テレポート**: プレイヤー名を入力して「テレポート」ボタン（またはEnterキー）を押すと、そのプレイヤーの位置にテレポートします。
    - **シングルプレイ**: 権限は不要です。統合サーバー（自分のPC内で動いている、同じプロセスのサーバー）に直接アクセスして相手プレイヤーのサーバー側エンティティを取得し、`ServerPlayNetworkHandler#requestTeleport` で直接動かしています。バニラのコマンド経由ではないため、「チートを許可」がOFFのワールドでも動作します。
    - **マルチプレイ（他人が管理するサーバー）**: サーバー側が位置を管理・検証しているため、クライアント側の処理だけでは権限チェックを回避できません。内部的にはバニラの `/tp <プレイヤー名>` コマンドを実行するため、引き続き**サーバーでOP権限が必要**です（権限が無い場合は「コマンドを使う権限がありません」等のメッセージがバニラ側からチャットに表示されます）。この制約は仕様であり、意図的に回避する実装はしていません。自分が管理するLANサーバー等であれば、自分にOPを付与すれば同様に使えます。
  - **フリーカム(ON/OFF)**: ボタン（または **Vキー**）でON/OFFを切り替えると、自分のキャラクターの位置・向きはその場に固定したまま、カメラだけを自由に動かして周りを見て回れます。
    - 移動: WASD（フリーカムの向き基準の前後左右）、Space で上昇、Shift で下降。Ctrl長押しで移動速度アップ。視点はマウスでそのまま操作できます。
    - 実際のキャラクターの位置・向きは変更しません。移動入力・視点回転をキャラクターに一切渡さず、毎tick速度を強制的にゼロへ戻しているので、他のプレイヤーから見ても自分はその場に立ち止まったまま（向きも変わりません）です。ネットワーク通信（パケット）には一切手を加えていません。
    - 壁や床などの当たり判定は無視して通り抜けられます（見た目の視点だけなので）。
    - 既知の制限: 一人称視点の手・アイテムの見た目は実際のプレイヤーの状態のまま描画されるため、フリーカム中は少し不自然に見えることがあります。
  ...
  - **ジャンプの高さ・フリーカムについての注意**: これらはバニラの内部メソッド（`getJumpVelocity` / `changeLookDirection` / `Camera#update`）を直接フックしており、Minecraftのバージョンアップでメソッド名が変わると効かなくなる可能性があります。その場合でも起動失敗はしない設計（`require = 0`）にしてあるので、効果が出ないだけで他の機能（移動速度・Velocity・ミニマップ・水上歩行など）は通常通り使えます（フリーカムの場合、視点は動かせてもキャラの向きが一緒に回転してしまう、または画面がフリーカムに切り替わらない、といった部分的な不具合になります）。
...
└── src/main/
    ...
    │   ├── MyCustomClient.java       # ClientModInitializer（キーバインド登録・入力処理・HUD登録・フリーカム移動）
    ...
    │   ├── FreecamController.java    # フリーカムON/OFF・仮想カメラ位置/向きの状態管理
    │   ├── gui/
    │   │   └── SpeedMenuScreen.java   # Mキーで開く設定メニュー(スライダー・ボタン)
    │   ├── hud/
    │   │   └── MinimapRenderer.java   # 右上のミニマップ描画
    │   └── mixin/
    │       ├── PlayerSpeedMixin.java   # 移動速度・Velocity・ジャンプ・水上歩行・フリーカム中の実キャラ固定を担当するMixin
    │       ├── FreecamLookMixin.java   # フリーカム中のマウス視点回転を実キャラではなく仮想カメラに反映するMixin
    │       ├── FreecamCameraMixin.java # 描画カメラの位置/向きをフリーカムの仮想カメラに差し替えるMixin
    │       └── LocalPlayerCheck.java   # 「自分自身のプレイヤーか」を判定する共通ヘルパー
    └── resources/
        ├── fabric.mod.json
