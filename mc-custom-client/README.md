...
- **設定メニュー（Mキー）**: ゲーム中に **M** を押すとスライダー付きの設定画面が開きます。ワールドは一時停止しないので、動かしながらその場で効果を確認できます。
  ...
  - **Velocity**: x0.50 〜 x2.50（初期値 x1.00）。ジャンプ・落下・ノックバックなど、実際の速度ベクトルそのものに毎tick掛かる倍率です。1.0より大きくすると全体的に「軽く、勢いよく」、小さくすると「重く、もっさり」動きます。暴走防止のため、速度の大きさは内部で頭打ち（クランプ）しています。
