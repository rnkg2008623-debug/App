...
10. **タイマー**: 作業内容にラベルを付けて計測できるタイマーです。他のタブに切り替えたりウィンドウが背面にあっても止まらず動き続け、停止すると作業時間が秒単位で記録されます。
11. **統計**: タイマーで記録した作業時間を秒・時間・日単位の合計で確認でき、3日〜3ヶ月の期間で日別のバーグラフとして可視化できます。
12. **学習**: フォルダを作成してクイズを整理できます。フォルダを選ぶと中のクイズが4択問題として出題され、正解・不正解を記録します。各フォルダの名前の横には、そのフォルダ内の全クイズの平均正答率が表示されます。
13. **インポート**: 学習タブ用のクイズをCSVファイル選択、またはテキストのコピー＆ペーストでまとめてインポートできます。「質問,選択肢1,選択肢2,選択肢3,選択肢4,正解の番号(1〜4)」の4択形式に加え、「単語, 意味」の単語帳形式（例: Apple, りんご）も貼り付けでき、他の単語の意味が自動でダミーの選択肢になり4択クイズになります（最低4語必要）。
